...
class ValidateDataQualityRulesBlock(ProcessingBlock):
    id = "data.quality.validate_rules"
    version = "0.1.0"
        ...
        for idx, it in enumerate(items):
            ...
            for rule in rules:  # type: ignore[assignment]
                ...
                if rtype == "required":
                    ...
                    if missing:
                        violations.append({"id": rid, "item_ref": ref, "type": rtype, "details": {"missing": missing}})
                elif rtype == "range":
                    v = _get(it, str(field))
                    try:
                    ...
                    if minv is not None and val < minv:
                        violations.append({"id": rid, "item_ref": ref, "type": rtype, "details": {"field": field, "min": minv, "actual": val}})
                    if maxv is not None and val > maxv:
                        violations.append({"id": rid, "item_ref": ref, "type": rtype, "details": {"field": field, "max": maxv, "actual": val}})
                elif rtype == "regex":
                    pattern = rule.get("pattern", ".*")
                    val = _get(it, str(field))
                    if val is None:
                        violations.append({"id": rid, "item_ref": ref, "type": rtype, "details": {"field": field, "reason": "missing"}})
                        continue
                    try:
                        if not re.search(pattern, str(val)):
                            violations.append({"id": rid, "item_ref": ref, "type": rtype, "details": {"field": field, "pattern": pattern, "val": val}})
                    except Exception:
                        continue
                elif rtype == "unique":
                    val = _get(it, str(field))
                    ...
                    if val in b:
                        violations.append({"id": rid, "item_ref": ref, "type": rtype, "details": {"field": field, "duplicate": val}})
                    else:
                        b.add(val)

        return {"violations": violations, "summary": {"items": len(items), "rules": len(rules), "violations": len(violations), "mode": "strict" if strict else "lenient"}}

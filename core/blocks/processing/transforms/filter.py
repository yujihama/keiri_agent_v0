...
class FilterBlock(ProcessingBlock):
    """Unified filter block with a minimal, strict condition schema.

    Inputs:
      - items: list[dict]
      - conditions: list[{ field: str, operator: enum[eq,ne,gt,gte,lt,lte,contains,in,between], value: any, value2?: any }]
      - options?: { case_insensitive?: bool }

    Outputs:
      - filtered: list[dict]
      - excluded: list[dict]
      - summary: { input: int, filtered: int, excluded: int }
    """

    id = "transforms.filter"
    version = "0.1.0"

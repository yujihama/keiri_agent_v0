...
注記: 本書は機能面に限定し、非機能（UX要件/可用性/セキュリティ/テスト/ROI等）は別資料に切り出します。コード断片は設計例であり、現行実装のパスに必ずしも一致しません。
...
### 2. 既存アーキテクチャとの統合
- 既存のUI構成（`ui/*`、特に `ui/tabs/*`）の拡張
- `ui/*` モジュールとの連携に統一（旧記載の `core/ui/*` は設計案）
- セッション状態管理の強化
...
<!-- 本書は機能面に限定。非機能や期待効果（UX/運用/可用性など）は別途検討に移すため省略 -->
